...
def metric(eye, eyebrow, height, width):
	...
	print("area is")
	print(area)
	distance = dist.euclidean(eye[2], eyebrow[2]) + \
		  dist.euclidean(eye[3], eyebrow[3]) + \
		  dist.euclidean(eye[4], eyebrow[4])
	den = height * width * distance

	print("metric is")
	print(metric)
	return np.tanh(float(num) / float(den))
...
while True:
	...
	for rect in rects:
		...
		cv2.putText(frame, "Left: {:.4f}".format(leftValue), (300, 30),
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

		cv2.putText(frame, "Right: {:.4f}".format(rightValue), (300, 60),
					cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
